    ...
    if t:
        ...
        for i in soup.find_all(attrs={'class': 'provincetr'}):
            for a in i.find_all('a'):
                ...
                # lists[id] = {'id': id, 'name': a.text, 'pid': '0', 'pid1': '0', 'pid2': '0', 'pid3': '0', 'pid4': '0','code': id}
                lists[id] = {'name': a.text, 'id': id}
                # time.sleep(1 / 10)
    return lists
    ...
    for i in l:
        ...
        for v in soup.find_all(attrs={'class': 'citytr'}):
            ...
            if id[0:4] not in lists.keys():
                # lists[id[0:4]] = {'id': id[0:4], 'name': str(v.find_all('td')[1].text),'pid': '0', 'pid1': i, 'pid2': '0', 'pid3': '0', 'pid4': '0', 'code': id}
                lists[id[0:4]] = {'id': id[0:4], 'name': str(v.find_all('td')[1].text), 'parentId' : id[0:2], "parentName": lists.get(id[0:2])['name']}
    return lists
...
p = provincetr(u=url, he=header, lists=dic)
print('省')
# for pvalue in p.values():
    # print(pvalue)
c = citytr(u=url, he=header, lists=p)
print('市')
for cvalue in c.values():
    print(cvalue)
