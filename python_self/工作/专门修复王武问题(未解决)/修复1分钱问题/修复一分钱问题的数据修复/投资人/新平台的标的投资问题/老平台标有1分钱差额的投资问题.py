...
def genSql():
    ...
    #          " from " \
    #          "(select amount,ifnull(sum(expect_principal),0) as money1,investment_id from invest.t_investment_%02d where asset_type = 0 and asset_pool = 1 and asset_property & 65536 = 65536 group by investment_id)  a," \
    ...
    #修复待收利息不符合
     ...
     #         " a.investment_id,a.amount,a.money1,b.money2 " \
     #         " from " \
     #         "(select amount,ifnull(sum(expect_interest),0) as money1,investment_id from invest.t_investment_%02d where asset_type = 0 and asset_pool = 1 and asset_property & 65536 = 65536 group by investment_id)  a," \
    ...
    #修复待收加息
     ...
     #           " a.investment_id,a.amount,a.money1,b.money2 " \
     #           " from " \
     #           "(select amount,ifnull(sum(expect_add_interest),0) as money1,investment_id from invest.t_investment_%02d where asset_type = 0 and asset_pool = 1 and asset_property & 65536 = 65536 group by investment_id)  a," \
    ...
    #预计平台管理费
     for idx in range(0, 100):
         sql = "select " \
               " a.investment_id,a.amount,a.money1,b.money2 " \
               " from " \
               "(select amount,ifnull(sum(expect_pay_platform),0) as money1,investment_id from invest.t_investment_%02d where asset_type = 0 and asset_pool = 1 and asset_property & 65536 = 65536 group by investment_id)  a," \
