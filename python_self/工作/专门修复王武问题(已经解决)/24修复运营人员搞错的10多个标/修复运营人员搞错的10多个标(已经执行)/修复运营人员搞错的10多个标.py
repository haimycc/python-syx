...
def genPayoffInfo():
    with open("./updatePyaoffSql2", "a+") as f:
                ...
                for asset in assets:
                    ...
                    for invest in investLists:
                        ...
                        for payoff in payoffs:
                            ...
                            if repayMode == 3:
                                repayMode = 5
                            benjin,lixi=payoffWay(amount,repayMode,phase,phaseMode,annualRate)
                            updateSql=str.format("update invest.t_investment_payoff_%s set expect_principal= %d,expect_interest=%d where asset_id = %d and payoff_id = %d and state =0 ; \n" % (str(payoffId)[-2:],benjin,lixi,assetId,payoffId))
                            f.write(updateSql)
                            f2.write("%d,%s,%d,%d,%d,%d,%d,%d\n" % (assetId,assetName,amount,phaseNum,expectPrincipal,expectInterest,benjin,lixi))
