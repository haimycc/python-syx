...
def getErrorData():
    with open("./3updateSql.20170301","a+") as f:
        for idx in range(0,100):
            ...
            for row in rows:
                ...
                phase=row["phase"]
                #找到payoff
                sql="select payoff_id from invest.t_investment_payoffs where asset_id=%d and repayment_id=%d and phase=%d " % (assetId,repayId,phase)
                new_cursor.execute(sql)
                payoffs = new_cursor.fetchall()
                for payoff in payoffs:
                    payoffId=payoff["payoff_id"]
                    payoffIdSuffix= int(str(payoffId)[-2:])
                    updateSql="update invest.t_investment_payoff_%02d " \
                           "set " \
                           "actual_interest=0," \
                           "actual_pay_platform=0 " \
                           "where " \
                           "asset_id=%d and repayment_id=%d and phase=%d ; \n" % (payoffIdSuffix,assetId,repayId,phase)
                    updateSqls="update invest.t_investment_payoffs " \
                           "set " \
                           "actual_interest=0," \
                           "actual_pay_platform=0 " \
                           "where " \
