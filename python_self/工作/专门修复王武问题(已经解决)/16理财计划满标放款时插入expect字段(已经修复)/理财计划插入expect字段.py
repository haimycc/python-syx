...
def updateExpect():
    with open(os.path.abspath(os.curdir) + '/updateExpect', 'a+') as f:
        sql = "select * from product.t_assets where asset_type = 1 and asset_property & 65536 != 65536  and state=8 ;"
        ...
        for result in results:
            ...
            investSql = str.format(
                "select investment_id,amount from product.t_investment_%s where asset_id=%d and asset_type=1 " % (
                assetIdSuffix, assetId))
            ...
            for invest in invests:
                ...
                sql1 = str.format(
                    "update invest.t_investment_%s set expect_interest=%d,expect_add_interest=%d  where asset_id = %d and asset_type = 1 and investment_id = %d and expect_interest=0 and expect_add_interest=0 ;" %
                    (investIdSuffix, expectInterest, expectAddInterest, assetId, investId))
                sql2 = str.format(
                    "update product.t_investment_%s set expect_interest=%d,expect_add_interest=%d  where asset_id = %d and asset_type = 1 and investment_id = %d and expect_interest=0 and expect_add_interest=0 ;" %
                    (assetIdSuffix, expectInterest, expectAddInterest, assetId, investId))
                sql3 = str.format(
                    "update financial_plan.t_investment_%s set expect_interest =%d,expect_add_interest=%d where asset_id= %d and asset_type = 1 and investment_id = %d and expect_interest=0 and expect_add_interest=0 ;" %
                    (assetIdSuffix, expectInterest, expectAddInterest, assetId, investId))
                sqls = str.format(
                    "update invest.t_investments set expect_interest=%d,expect_add_interest=%d  where asset_id = %d and asset_type = 1  and investment_id = %d and expect_interest=0 and expect_add_interest=0; " %
