...
def save_state(cursor, state):
    ...
    cursor.execute(
        INSERT_STATE,
        (state.location_name, json.dumps([x.name for x in state.inventory])))
    for loc in state.world.values():
        items = json.dumps(thaw(loc.items.keys()))
        exits = serialize_exits(loc.exits)
        cursor.execute(INSERT_LOCATION, (loc.name, loc.description, exits, items))
...
def load_state(cursor):
    location_rows = cursor.execute(SELECT_LOCATION)
    world = {row[0]: load_location(row) for row in location_rows}
